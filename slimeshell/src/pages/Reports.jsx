...
export default function Reports() {
  ...
  return (
    <div className="flex gap-4 h-[calc(100vh-120px)]">
      {/* Config Panel */}
      <div className="w-[380px] flex-shrink-0 flex flex-col gap-3 overflow-auto">
        <Card>
          <span className="font-mono text-[10px] font-semibold uppercase text-text-dim block mb-3">Report Config</span>
          <div className="space-y-3">
            <Input label="Report Title" value={title} onChange={(e) => setTitle(e.target.value)} />
            <div>
              <label className="block font-mono text-[10px] font-semibold uppercase text-text-dim mb-1.5">Report Type</label>
              <select
                value={type}
            ...
            <Input label="Tester" value={tester} onChange={(e) => setTester(e.target.value)} />
        ...
        <Card>
          <div className="flex items-center justify-between mb-3">
            <span className="font-mono text-[10px] font-semibold uppercase text-text-dim">Findings ({findings.length})</span>
              ...
              return (
                <div key={finding.id} className="bg-slime-code rounded-md p-2.5 hover:bg-white/[0.02] transition-colors">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <button onClick={() => removeFinding(finding.id)} className="text-text-faint hover:text-rose cursor-pointer">
                      <Trash2 size={10} />
                    </button>
                  </div>
                  <div className="flex items-center gap-2">
                    <Badge color={severityColors[finding.severity]}>{finding.severity}</Badge>
                    <span className="font-mono text-[9px] text-text-faint">CVSS {finding.cvss}</span>
                  </div>
                </div>
              )
            })}
        ...
        <div className="flex gap-2">
          <Button variant="primary" className="flex-1"><Download size={14} /> Export PDF</Button>
          <Button variant="ghost"><Eye size={14} /></Button>
        </div>
      </div>
      ...
      <Card className="flex-1 flex flex-col min-w-0">
        <div className="flex items-center justify-between mb-3">
          <span className="font-mono text-[10px] font-semibold uppercase text-mint">Live Preview</span>
          <Badge color="mint" pill>{type}</Badge>
        </div>

        <div className="flex-1 bg-slime-code rounded-md border border-white/[0.04] overflow-auto p-6">
          {/* Report Header */}
          <div className="border-b border-white/[0.06] pb-4 mb-4">
            <div className="font-mono text-[9px] text-text-faint uppercase mb-2">Confidential</div>
            ...
            <div className="grid grid-cols-4 gap-2">
              {[
                ...
                <div key={s.label} className="bg-slime-terminal rounded-md p-2 text-center">
                  ...
                  <div className="font-mono text-[9px] text-text-faint">{s.label}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            {findings.map((finding, i) => (
              <div key={finding.id} className="mb-4 border border-white/[0.04] rounded-md p-3">
                <div className="flex items-center gap-2 mb-2">
                  <span className="font-mono text-[10px] text-text-faint">F-{String(i + 1).padStart(3, '0')}</span>
                  <h3 className="font-heading font-semibold text-[14px] text-text-primary flex-1">{finding.title}</h3>
                  <Badge color={severityColors[finding.severity]}>{finding.severity}</Badge>
                  <span className="font-mono text-[10px] text-gold">CVSS {finding.cvss}</span>
                </div>
