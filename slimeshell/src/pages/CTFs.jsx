    ...
    <div className="space-y-4">
      ...
      <Card>
        <div className="flex items-center justify-between mb-3">
          <div>
            <span className="font-mono text-[10px] font-semibold uppercase text-mint">Active CTF</span>
            ...
            <div className="flex items-center gap-3 mt-1">
              ...
              <span className="font-mono text-[10px] text-text-dim">{activeCTF.startDate} — {activeCTF.endDate}</span>
            </div>
          </div>
          <div className="text-right">
            ...
            <div className="font-mono text-[9px] text-text-faint mt-0.5">remaining</div>
          </div>
        </div>

        <div className="grid grid-cols-4 gap-3 mb-3">
          <div className="bg-slime-code rounded-md p-2.5">
            <div className="font-mono text-[9px] text-text-dim uppercase">Rank</div>
            <div className="font-heading font-bold text-[22px] text-mint">#{activeCTF.rank}</div>
            <span className="font-mono text-[9px] text-text-faint">of {activeCTF.totalTeams}</span>
          </div>
          <div className="bg-slime-code rounded-md p-2.5">
            <div className="font-mono text-[9px] text-text-dim uppercase">Points</div>
            <div className="font-heading font-bold text-[22px] text-lavender">{activeCTF.points}</div>
          </div>
          <div className="bg-slime-code rounded-md p-2.5">
            <div className="font-mono text-[9px] text-text-dim uppercase">Solved</div>
          ...
          <div className="bg-slime-code rounded-md p-2.5">
            <div className="font-mono text-[9px] text-text-dim uppercase">Team</div>
        ...
        <div className="flex items-center justify-between">
          <span className="font-mono text-[9px] text-text-faint">Challenge Progress</span>
          <span className="font-mono text-[10px] text-mint">{Math.round((activeCTF.progress / activeCTF.total) * 100)}%</span>
        </div>
      ...
      <Card>
        <div className="flex items-center justify-between mb-3">
          <span className="font-mono text-[10px] font-semibold uppercase text-text-dim">Challenges</span>
            ...
            <div className="flex gap-1">
              {['all', 'solved', 'unsolved'].map((s) => (
                <button
                  ...
                  className={`px-2.5 py-1 rounded-md font-mono text-[10px] transition-colors cursor-pointer
            ...
            <div className="flex gap-1">
              {categoryFilter.map((cat) => (
                <button
                  ...
                  className={`px-2 py-1 rounded-md font-mono text-[10px] transition-colors cursor-pointer
        ...
        <div className="space-y-1">
          {filteredChallenges.map((ch) => (
            <div key={ch.id} className="flex items-center gap-3 py-2 px-2.5 rounded-md hover:bg-white/[0.02] transition-colors">
              ...
              {ch.solved ? (
                <>
                  <span className="font-mono text-[9px] text-text-dim w-20 text-right">{ch.solvedBy}</span>
                  <span className="font-mono text-[9px] text-text-faint w-14 text-right">{ch.time}</span>
                </>
              ) : (
                <span className="font-mono text-[9px] text-rose w-36 text-right">Unsolved</span>
              )}
            </div>
          ))}
        </div>
      ...
      <Card>
        ...
        <div className="space-y-1">
          {completedCTFs.map((ctf) => (
            <div key={ctf.name} className="flex items-center gap-3 py-2.5 px-2.5 rounded-md hover:bg-white/[0.02] transition-colors">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="font-mono text-[9px] text-text-faint">{ctf.date} · {ctf.platform}</div>
              </div>
              <div className="text-right mr-2">
                <div className="font-heading font-bold text-[14px] text-mint">#{ctf.rank}</div>
                <div className="font-mono text-[8px] text-text-faint">of {ctf.total}</div>
              </div>
              <div className="text-right mr-2">
                <div className="font-mono text-[11px] text-text-primary">{ctf.solved}/{ctf.outOf}</div>
                <div className="font-mono text-[8px] text-text-faint">solved</div>
              </div>
              <div className="text-right">
