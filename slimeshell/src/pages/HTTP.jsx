    ...
    <div className="flex flex-col gap-4 h-[calc(100vh-120px)]">
      ...
      <div className="flex gap-4 flex-1 min-h-0">
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <Card className="flex-1 mt-3 flex flex-col overflow-auto">
              ...
              <div className="space-y-2">
                  ...
                  <div key={i} className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      className="flex-[2] bg-slime-code border border-white/[0.04] rounded-md px-2.5 py-1.5
                        ...
                        focus:outline-none focus:border-mint/15"
                    />
                    <button onClick={() => removeHeader(i)} className="text-text-faint hover:text-rose cursor-pointer p-1">
                      <Trash2 size={12} />
                    </button>
                  </div>
                ))}
                <button
                  ...
                  className="w-full border border-dashed border-white/[0.08] rounded-md py-2 text-text-dim
                    font-mono text-[10px] hover:border-mint/20 hover:text-mint transition-colors cursor-pointer
            ...
            {activeBodyTab === 'body' && (
              <textarea
                ...
                className="flex-1 bg-slime-code rounded-md border border-white/[0.04] p-3 font-mono text-[11px]
                  text-mint resize-none focus:outline-none focus:border-mint/15 whitespace-pre"
                spellCheck={false}
              />
            )}
            ...
            {activeBodyTab === 'params' && (
              <div className="font-mono text-[11px] text-text-dim p-4">
                ...
                <div className="mt-2 bg-slime-code rounded-md p-2.5 border border-white/[0.04]">
                  <span className="text-text-faint text-[10px]">No query parameters detected</span>
                </div>
              </div>
            )}
            ...
            {activeBodyTab === 'auth' && (
              <div className="space-y-3 p-1">
                <div>
                  <label className="block font-mono text-[10px] font-semibold uppercase text-text-dim mb-1.5">Auth Type</label>
                  <select className="w-full bg-slime-code border border-white/[0.04] rounded-md px-3 py-2
                ...
                <div>
                  <label className="block font-mono text-[10px] font-semibold uppercase text-text-dim mb-1.5">Token</label>
                  <input
          ...
          {/* History */}
          <Card className="mt-3">
            <span className="font-mono text-[10px] font-semibold uppercase text-text-dim block mb-2">Recent Requests</span>
            <div className="space-y-1">
              {requestHistory.map((req, i) => (
                <div key={i} className="flex items-center gap-2 py-1.5 px-2 rounded-md hover:bg-white/[0.02] transition-colors cursor-pointer">
                  <Badge color={methodColors[req.method]}>{req.method}</Badge>
                  <span className="font-mono text-[10px] text-text-secondary flex-1 truncate">{req.url}</span>
                  <Badge color={statusColor(req.status)}>{req.status}</Badge>
                  <span className="font-mono text-[9px] text-text-faint">{req.time}</span>
                </div>
              ))}
            </div>
          </Card>
            ...
            <>
              <div className="flex items-center gap-3 mb-3">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span className="font-mono text-[10px] text-text-dim">23ms</span>
                </div>
                <span className="font-mono text-[10px] text-text-dim">342 bytes</span>
              </div>
                  ...
                  <div className="space-y-1">
                    {sampleResponseHeaders.map((h, i) => (
                      <div key={i} className="flex items-center gap-3 py-1.5 px-2 rounded-md hover:bg-white/[0.02]">
                        <span className="font-mono text-[10px] text-text-dim w-48">{h.key}</span>
                        <span className="font-mono text-[10px] text-text-secondary flex-1 truncate">{h.value}</span>
                      </div>
                    ))}
                  </div>
                )}
